#!/usr/bin/env python3
"""Safely upload a LibTV reference or download generated media."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    download.add_argument("--without-ai-watermark", action="store_true")
    return parser
...
def main() -> int:
    ...
    if args.operation == "upload":
        ...
        if not resource.is_file():
            print(f"Reference file does not exist: {resource}", file=sys.stderr)
            return 2
        command = [
            libtv_bin,
            "upload",
